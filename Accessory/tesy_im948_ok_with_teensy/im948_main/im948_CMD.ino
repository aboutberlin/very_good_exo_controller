...
static void Cmd_RxUnpack_Left(U8 *buf, U8 DLen);   
static void Cmd_RxUnpack_Right(U8 *buf, U8 DLen);   
/**
 * 发送CMD命令
 *
 * @param pDat 要发送的数据体
...
int Cmd_PackAndTx_Left(U8 *pDat, U8 DLen)
{
    U8 buf[50 + 5 + CmdPacketMaxDatSizeTx] =
        {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    Cmd_Write_Left(buf, DLen+55);
    return 0;
}

int Cmd_PackAndTx_Right(U8 *pDat, U8 DLen)
{
    U8 buf[50 + 5 + CmdPacketMaxDatSizeTx] =
        {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        {// 捕获到完整包
            ...
            {// 地址匹配，是目标设备发来的数据 才处理
                Dbp_U8_buf("rx: ", "\r\n",
                           "%02X ",
                           buf, i);
                Cmd_RxUnpack_Left(&buf[3], i-5);   // 处理数据包的数据体
                return 1;
            }
        }
        break;
        ...
        {// 捕获到完整包
            ...
            {// 地址匹配，是目标设备发来的数据 才处理
                Dbp_U8_buf("rx: ", "\r\n",
                           "%02X ",
                           buf, i);
                Cmd_RxUnpack_Right(&buf[3], i-5);   // 处理数据包的数据体 
                return 1; 
            }
        }
        break;
...
void Cmd_12_Left(U8 accStill, U8 stillToZero, U8 moveToZero,  U8 isCompassOn, U8 barometerFilter, U8 reportHz, U8 gyroFilter, U8 accFilter, U8 compassFilter, U16 Cmd_ReportTag)
{
    U8 buf[11] = {0x12};
    buf[1] = accStill;
    buf[2] = stillToZero;
    ...
    buf[10] = (Cmd_ReportTag>>8)&0xff;
    Dbp("\r\nset parameters--\r\n");
    Cmd_PackAndTx_Left(buf, 11);
}

void Cmd_12_Right(U8 accStill, U8 stillToZero, U8 moveToZero,  U8 isCompassOn, U8 barometerFilter, U8 reportHz, U8 gyroFilter, U8 accFilter, U8 compassFilter, U16 Cmd_ReportTag)
{
    U8 buf[11] = {0x12};
    buf[1] = accStill;
    buf[2] = stillToZero;
 ...
 * 设置 陀螺仪自动校正系数
 *
 * @param GyroAutoVal  系数值
 */
void Cmd_35(U8 GyroAutoVal)
{
    U8 buf[2] = {0x35};
    buf[1] = GyroAutoVal;
    Dbp("\r\nset GyroAutoVal--\r\n");
...
static void Cmd_Write(U8 *pBuf, int Len)
{
    SerialImuLeft.write(pBuf, Len); // 通过该函数发送通信数据流
...
static void Cmd_Write_Left(U8 *pBuf, int Len)
{
    SerialImuLeft.write(pBuf, Len); // 通过该函数发送通信数据流
...
static void Cmd_Write_Right(U8 *pBuf, int Len)
{
    SerialImuRight.write(pBuf, Len); // 通过该函数发送通信数据流
...
void im948_test(void)
{
    ...
    case 'M': // M 设置陀螺仪自动校正系数
        Cmd_35(1); // 设为1(这也是出厂默认值)
        break;
    case 'N': // N 设置陀螺仪自动校正系数
        Cmd_35(4); // 设为4
        break;
