...
#define __Debug  // 使用调试口输出调试信息,不使用调试信息屏蔽本句即可
    ...
    #define Dbp(fmt, args...)  myPrintf(fmt, ##args) // 若需要使用调试信息, 用户对接Dbp函数名即可
    extern void Dbp_U8_buf(const char *sBeginInfo, const char *sEndInfo, char *sFormat, const U8 *Buf, U32 Len);
#else
    #define Dbp(fmt, args...)
 ...
 */
extern void Cmd_35(U8 GyroAutoVal);
