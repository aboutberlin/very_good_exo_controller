// Teensy 4.1 使用 Serial6: TX2 (pin 47), RX2 (pin 48)
...
void setup() {
  ...
  // 初始化 TR100 通信串口
  Serial6.begin(460800); // TR100 默认波特率为 460800

  delay(1000); // 等待模块上电稳定（1秒内发送指令）

  // === 设置为主机绑定模式 ===
  Serial6.write("AT+BleSetToClientModeFind....."); // 补齐30字节
  Serial.println("已发送：主机绑定模式指令");

  delay(100); // 等待模块处理

  // === 设置蓝牙名称为 TR100（可选） ===
  Serial6.write("AT+BleSetName:TR100==========="); // 蓝牙名称最多15字节，用 = 补足
